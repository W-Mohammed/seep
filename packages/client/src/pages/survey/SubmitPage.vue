<template>
  <div class="flex flex-col items-center justify-center md:mt-10 lg:mt-30 pb-24">
    <div class="w-full max-w-60ch sm:rounded ">
      <div class="text-2xl text-center font-bold break-words text-pretty">
        
        <div class="mb-10">
          {{ content?.title ?? 'Ready to submit your responses?' }}
        </div>
      </div>
      <div class="p-4">
        <div class="flex flex-col gap-4 text-center break-words text-pretty">
          <p class="text-lg">
            {{ content?.body ?? 'You have completed all the questions. Are you ready to submit your responses?' }}
          </p>
          <p class="inline" v-if="content?.note">
            <span class="inline">
              <Icon icon="bi:exclamation-triangle" class="pb-1 text-lg inline-block text-gray-900" />
              {{ content.note ?? "Note: You won't be able to change your responses afterwards.." }}
            </span>
          </p>
        </div>
      </div>
      <div class="flex justify-center py-2">
        <button v-if="!loading" class="w-40 mt-2 px-1.5 text-white text-sm flex hover:opacity-90 items-center py-3 text-md bg-black font-medium" @click="submitProxy()">
          {{content.btnSubmitLabel ?? 'Submit'}}
        </button>
        <button
          v-else
          disabled
