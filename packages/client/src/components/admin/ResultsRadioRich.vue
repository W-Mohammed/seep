...
const responseByExpert = computed(() => {
 return  RadioRichAgendaItem.getResponse(currentActiveExperts.value, selectedQuestion.value)
...
const options = computed(() => {
  ...
  const currentQuestion = currentSurveyQuestions?.value.filter((q) => q._id === selectedQuestion.value)[0]
  const responseOpts = currentQuestion?.content.options.map((opt) => opt.title)

  const colors = viridis(responseOpts.length + 2)
  const colorMap = {}
  responseOpts?.forEach((cat, i) => {
    colorMap[cat] = colors[i]
  })

  
  return {
    color: Object.keys(responseStats.value).map((cat) => colorMap[cat]),
    tooltip: {
      trigger: 'item',
      formatter: '{b}<br> {c} ({d}%)'
    },
    series: [
      {
        ...
        data: Object.entries(responseStats.value).map(([name, value]) => ({ name, value })),
        label: {
          show: true,
          formatter: (params) => {
