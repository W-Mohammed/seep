...
const responseByExpert = computed(() =>
 QuestionFormKitAgendaItem.getResponse(currentActiveExperts.value, selectedQuestion.value)
)
...
const options = computed(() => {
  ...
  let responseOpts = Object.keys(responseStats.value)
  if (currentSurveyQuestions?.value && selectedQuestion?.value) {
    responseOpts = currentSurveyQuestions?.value
  ...
  responseOpts?.forEach((cat) => {
    if (!categories.includes(cat)) {
      categories.push(cat)
      responseStats.value[cat] = 0
    }
  })
